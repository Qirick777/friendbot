...
public class BotKiteFlipTest implements BotTest {
    ...
    public String scenarioSpec() {
        return "setBaseValue(MAX_HEALTH) 호출값: 200.0 (봇/표적 구분은 setup() 참조). bot.setInvulnerable(false). 스폰 "
                + "몹: zombie. 관측 창과 트라이얼 수는 리터럴이 아니라 상수 계산식이다(timeoutTicks()/repeats() 참조). 유저 없음(TestUser.spawn 미호출) → Perception.java:131이 봇 외 플레이어를 "
                + "찾지 못해 user==null → BotIdle.java:87-89 즉시 반환. 16장 자율 이동 비활성. ";
    }
