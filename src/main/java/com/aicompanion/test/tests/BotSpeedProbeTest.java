...
public class BotSpeedProbeTest implements BotTest {
    ...
    public String scenarioSpec() {
        return "봇 최대체력 400 + 매 틱 만피 회복 — 속도만 재려고 사망·생존 분기를 배제한 전제. 추격자는 setInvulnerable(true). 유저 없음(TestUser.spawn 미호출) → Perception.java:131 level.players()가 봇 외 플레이어를 찾지 못해 perception().user==null → BotIdle.java:87-89 즉시 반환. 16장 자율 이동 비활성이며 판정 라인의 idleCommandedTicks:0로 값 확인된다(O-5(2)).";
    }
