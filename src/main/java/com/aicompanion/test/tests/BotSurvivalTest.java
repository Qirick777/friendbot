...
public class BotSurvivalTest implements BotTest {
    ...
    public String scenarioSpec() {
        return "setBaseValue(MAX_HEALTH) 호출값: 20.0, 200.0 (봇/표적 구분은 setup() 참조). "
                + "bot.setInvulnerable(true). 스폰 몹: zombie. 관측 400틱, 트라이얼 1회. 시공 범위 선언: {-12, ENEMY_DX + 3, "
                + "-4, 4}. 유저 없음(TestUser.spawn 미호출) → Perception.java:131이 봇 외 플레이어를 찾지 못해 user==null → "
