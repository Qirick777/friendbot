...
public class BotMeleeTest implements BotTest {
    ...
    public String scenarioSpec() {
        return "setBaseValue(MAX_HEALTH) 호출값: 2000.0 (봇/표적 구분은 setup() 참조). bot.setInvulnerable(true). 스폰 "
                + "몹: zombie. 관측 400틱, 트라이얼 3회. 시공 범위 선언: {-3, 8, -3, 3}. 유저 없음(TestUser.spawn 미호출) → "
